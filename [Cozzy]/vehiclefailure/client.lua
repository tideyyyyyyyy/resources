...
AddEventHandler('vehiclefailure:client:RepairVehicle', function()
	local vehicle = XD.Functions.GetClosestVehicle()
...
function CleanVehicle(vehicle)
	...
	XD.Functions.Progressbar("repair_vehicle", "Repairing..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		XD.Functions.Notify("Voertuig schoon!")
		SetVehicleDirtLevel(vehicle)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('vehiclefailure:server:removewashingkit')
		TriggerEvent('inventory:client:ItemBox', XD.Shared.Items["cleaningkit"], "remove")
...
function RepairVehicleFull(vehicle)
	...
	XD.Functions.Progressbar("repair_vehicle", "Rapairing vehicle..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		XD.Functions.Notify("Vehicle Repaired!")
		...
		TriggerServerEvent('xd_vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		XD.Functions.Notify("Canceled!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	...
	XD.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('xd_vehiclefailure:removeItem', "repairkit")
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		XD.Functions.Notify("Canceled!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            XD.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['xd_vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['xd_vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	--exports['xd_vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['xd_vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end
